...
export default function ChatbotFloatingButton() {
  ...
  className="relative shadow-[0_20px_50px_rgba(0,0,0,0.2)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden border border-white/40 dark:border-slate-700/50"
  style={{
    position: "fixed",
    zIndex: 2147483646,
    ...(isMobile
      ...
      : {
          ...
          width: 380,
          height: "min(655px, 75vh)",
          borderRadius: 28,
        }),
  }}
>
  {/* Safari-safe glass layer */}
  <div
    className="absolute inset-0 bg-white/10 dark:bg-slate-900/50 pointer-events-none"
    style={{
  ...
  <div className="relative z-10 flex h-full flex-col">
    {/* Header */}
    <div className="relative p-4 flex items-center justify-between shrink-0 bg-white/10 dark:bg-slate-900/75 border-b border-slate-200/80 dark:border-slate-800">
      <div className="flex items-center gap-3 w-full">
        ...
        <div className="relative shrink-0">
          <div className="w-10 h-10 rounded-full overflow-hidden bg-white/30 dark:bg-white/5 border border-indigo-200 dark:border-indigo-800">
            {avatarUrl ? (
              ...
              <div className="w-full h-full flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400">
                K
              </div>
            )}
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-slate-900 dark:text-white truncate text-sm leading-tight">
            {botName}
          </div>
        ...
        <div className="flex bg-slate-100/80 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 text-[10px] font-bold rounded-lg transition-all ${
              mode === "smart"
                ? "bg-white/80 dark:bg-white shadow-sm text-indigo-600 dark:text-indigo-400 border border-indigo-600 dark:border-indigo-600"
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 text-[10px] font-bold rounded-lg transition-all ${
              mode === "pro"
                ? "bg-white/80 dark:bg-indigo-600 shadow-sm text-red-500 dark:text-red-800 border border-red-400 dark:border-red-700"
                : "text-slate-400 dark:text-slate-500"
            }`}
          >
        ...
        <button
          ...
          className="ml-1 p-2 hover:bg-slate-100/70 dark:hover:bg-slate-800/70 rounded-lg text-slate-400 transition-colors"
        >
          <X size={18} />
        </button>
      </div>
    ...
    {/* Chat Body */}
    <div className="flex-1 relative bg-white/5 dark:bg-slate-950/40 overflow-hidden">
